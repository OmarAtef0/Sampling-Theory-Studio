...
def add_noise(self, noise_slider_value):
    ...
    if self.graph_empty:
        ...
        if noise_slider_value == 0:
            ...
            noisy_amplitude = signal_amplitude + noise * noise_slider_value / 10
...
def move_to_viewer(self, Input):
    ...
    self.ui.sampling_slider.setMaximum(5 * int(self.current_signal.max_analog_freq))
    self.ui.sampling_slider_1.setMaximum(5)
    self.ui.fmaxLCD.display(int(self.current_signal.max_analog_freq))
    print("fmax: ", int(self.current_signal.max_analog_freq))
...
def refresh_graphs(self):
    ...
    if len(self.current_signal.amplitude) > 0 and len(self.reconstructed_amplitude) > 0:
        ...
        if self.ui.noise_slider.value() > 0:
            self.error *= 0.65
            self.error = np.convolve(self.error, np.ones(5)/5, mode='same')
...
def refresh_graphs(self):
    ...
    if len(self.current_signal.amplitude) > 0 and len(self.reconstructed_amplitude) > 0:
        ...
        if self.fmax_boolean == True and self.ui.noise_slider.value() > 1:
            self.fraction_val += 0.1
            self.error *= self.fraction_val
...
def change_sampling_rate(self, freqvalue):
  if self.graph_empty:
    ...
    if self.ui.noise_slider.value() > 38:
        self.fmax_boolean = True
